...
def getJSON(filepath):
    Patient = json.loads(getPatientInfo(filepath))
    AnalyseInfo = json.loads(getAnalyzeInfo(filepath))
    MeasureInfo = json.loads(getMeasureInfo(filepath))
    StimTimes = json.loads(getStimTimes(filepath))

    StimT = {'Attribute': 'StimTimes', 'Values' : str(StimTimes)}

    MeasureInfo.append(StimT)

    JSON = {'PatientInformation': str(Patient), 'AnalyseInformation' : str(AnalyseInfo), 'MeasureInformation' : str(MeasureInfo)}
...
#with open('test.json', 'w') as f:
 #   json.dump(getJSON('Dataset1_VM/VMData/VM0003_ViMo_HBA_Probe1_Total.csv'), f, indent = 4)
